...
uint8_t app_a2dp_player_playback_delay_aac_mtu = 3; 
uint8_t app_a2dp_player_playback_max_waterline_aac_mtu = 24;
uint8_t app_a2dp_player_playback_min_waterline_aac_mtu = 0;
#define A2DP_PLAYER_PLAYBACK_DELAY_AAC_BASE (23000)
uint32_t app_a2dp_player_playback_delay_aac_us = A2DP_PLAYER_PLAYBACK_DELAY_AAC_BASE*app_a2dp_player_playback_delay_aac_mtu;


uint8_t app_a2dp_player_playback_delay_sbc_frame_mtu =7;
uint8_t app_a2dp_player_playback_delay_sbc_mtu = 35;
uint8_t app_a2dp_player_playback_max_waterline_sbc_mtu = 25*app_a2dp_player_playback_delay_sbc_frame_mtu;//A2DP_PLAYER_PLAYBACK_DELAY_SBC_FRAME_MTU;
uint8_t app_a2dp_player_playback_min_waterline_sbc_mtu = 0*app_a2dp_player_playback_delay_sbc_frame_mtu;//A2DP_PLAYER_PLAYBACK_DELAY_SBC_FRAME_MTU;
#define A2DP_PLAYER_PLAYBACK_DELAY_SBC_BASE (2800)
uint8_t app_a2dp_player_playback_delay_sbc_us = A2DP_PLAYER_PLAYBACK_DELAY_SBC_BASE*app_a2dp_player_playback_delay_sbc_mtu;
...
extern APP_STREAM_GET_A2DP_PARAM_CALLBACK app_stream_get_a2dp_non_type_callback;
    ...
    if(bt_sbc_player_get_codec_type() == BT_A2DP_CODEC_TYPE_NON_A2DP){
    ...
    if(bt_sbc_player_get_codec_type() == BT_A2DP_CODEC_TYPE_MPEG2_4_AAC){
        tg_trigger_time = app_a2dp_player_playback_delay_aac_us * a2dp_audio_latency_factor_get();
...
#endif
#if (A2DP_DECODER_VER < 2)
        a2dp_audio_set_mtu_limit(app_a2dp_player_playback_delay_aac_mtu);
#endif
    }else
#endif
#if defined(A2DP_LHDC_ON)
    if(bt_sbc_player_get_codec_type() == BT_A2DP_CODEC_TYPE_NON_A2DP && nonType == A2DP_NON_CODEC_TYPE_LHDC){
    ...
    if(bt_sbc_player_get_codec_type() == BT_A2DP_CODEC_TYPE_NON_A2DP && nonType == A2DP_NON_CODEC_TYPE_LHDCV5){
        if (a2dp_lhdc_config_llc_get()){
...
#endif
    {
        uint32_t frame_mtu = app_a2dp_player_playback_delay_sbc_frame_mtu;
#if (A2DP_DECODER_VER == 2)
        A2DP_AUDIO_LASTFRAME_INFO_T lastframe_info;
        if (!a2dp_audio_lastframe_info_get(&lastframe_info)){
            frame_mtu = lastframe_info.totalSubSequenceNumber;
...
#endif
        tg_trigger_time = app_a2dp_player_playback_delay_sbc_us * a2dp_audio_latency_factor_get();
#ifdef CUSTOM_BITRATE
...
#if (A2DP_DECODER_VER < 2)
        a2dp_audio_set_mtu_limit(app_a2dp_player_playback_delay_sbc_mtu);
#endif
    }
