...
#define __INLINE__ __STATIC __INLINE
...
__STATIC int isValidSecretKey_256(u_int8* secretKey);
...
__INLINE__ void shiftLeftOneArrayElement256(bigHex256 *input)
{
    memmove(input->num,(input->num+1), (8*sizeof(u_int32)));
}
...
uint8_t ecc_generate_new_key256(u_int8 key_type, const u_int8* secret_key, const u_int8* public_key_x, const u_int8* public_key_y, u_int8* output)
{
    uint8_t status = BT_STS_INVALID_PARM;

    u_int32 big_num_offset=1;
    int32_t i, j;
    bigHex256 PrivateKey256={0};
    ...
    if (ecc_is_valid_point(&PublicKey256.x,&PublicKey256.y))
    {
        ...
        memcpy(output,(uint8_t *)&ind.key_res_x[0],PUB_KEY_256_LEN);
...
{
    ecc_generate_new_key256(ECC_PUBLICKEY_GENERATION,secret_key, BasePoint_x_256, BasePoint_y_256, out_public_key);
}

void ecc_gen_new_secret_key_256(uint8_t* secret_key256, bool forced)
{
...
//        }
    ...
    else
    {
        ...
        do
        {
            *(uint32_t *)(&secret_key256[0]) = (uint32_t)rand();
            *(uint32_t *)(&secret_key256[4]) = (uint32_t)rand();
            *(uint32_t *)(&secret_key256[8]) = (uint32_t)rand();
            *(uint32_t *)(&secret_key256[12]) = (uint32_t)rand();
            *(uint32_t *)(&secret_key256[16]) = (uint32_t)rand();
            *(uint32_t *)(&secret_key256[20]) = (uint32_t)rand();
            *(uint32_t *)(&secret_key256[24]) = (uint32_t)rand();
            *(uint32_t *)(&secret_key256[28]) = (uint32_t)rand();
        }
