...
void btif_btgatt_client_create(const bt_bdaddr_t *remote);
...
#if defined(IBRT)
uint32_t btif_btgatt_profile_save_ctx(const bt_bdaddr_t *remote, uint8_t *buf, uint32_t buf_len);
uint32_t btif_btgatt_profile_restore_ctx(const bt_bdaddr_t *remote, uint8_t *buf, uint32_t buf_len);
