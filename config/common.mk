...
export LC3_IN_ROM_V2 ?= 0
ifeq ($(LC3_IN_ROM_V2),1)
KBUILD_CPPFLAGS += -DLC3_IN_ROM_V2
endif
...
export BLE_GATT_CLIENT_CACHE ?= 0
...
export BLUETOOTH_BTH_DIR_PATH ?= bthost

export BLUETOOTH_ADAPTER_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/adapter
export BLUETOOTH_APP_DIR_COMMON = $(BLUETOOTH_BTH_DIR_PATH)/service/common
export BT_APP_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/service/bt_app
export BT_SOURCE_APP_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/service/bt_source
export BT_IF_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/bt_if
export BT_PROFILES_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/bt_profiles
export BT_STACK_ROM_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/rom
export BT_ROM_PORTING_PATH = $(BLUETOOTH_BTH_DIR_PATH)/porting
export BLE_AUDIO_USB_DIR_PATH = apps/ble_usb_audio
export BT_ROM_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/rom
export BT_ROM_DBG_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/dbg
export BT_ROM_PATCH_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/common/patch

export BLE_AUDIO_APP_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/service/ble_audio
export BLE_AOB_APP_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/service/ble_audio/aob_app

export BLE_APP_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/service/ble_app_new
export BLE_GAF_CORE_DIR_PATH = $(BLE_AUDIO_APP_DIR_PATH)/blueelf_adapter
export BLE_IF_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/ble_if
export BLE_STACK_DIR_PATH = $(BLUETOOTH_BTH_DIR_PATH)/stack/ble_stack_new
...
export BTH_IN_ROM ?= 0
ifeq ($(BTH_IN_ROM),1)
# workaround: force enable BLE to make sure gap global handles are initialized
export BLE := 1
KBUILD_CPPFLAGS += -DBTH_IN_ROM
endif

ifeq ($(BT_PLD_SIMU),master)
	KBUILD_CPPFLAGS += -DPLD_SIMU_MASTER
endif
ifeq ($(BT_PLD_SIMU),slave)
	KBUILD_CPPFLAGS += -DPLD_SIMU_SLAVE
endif

ifeq ($(BT_SOURCE_SIMU),1)
KBUILD_CPPFLAGS += -DBT_SOURCE_SIMU
endif
...
export POWER_ON_OPEN_BOX_ENABLED ?= 1
...
KBUILD_CPPFLAGS += -Iapps/nv_section/userdata_section
endif
...
NEW_IMAGE_FLASH_OFFSET ?= 0xF0000
...
#CHIP_CAPSENSOR_VER:4 =>1503
#CHIP_CAPSENSOR_VER:5 =>1307
ifeq ($(CHIP),best1306)
CHIP_CAPSENSOR_VER ?= 3
else ifeq ($(CHIP),best1503)
CHIP_CAPSENSOR_VER ?= 4
else ifeq ($(CHIP),best1307)
CHIP_CAPSENSOR_VER ?= 5
else
...
KBUILD_CFLAGS += -DCAPSENSOR_FAC_CALCULATE
endif
export CAPSENSOR_READ_DATA_POLLING ?=1
ifeq ($(CAPSENSOR_READ_DATA_POLLING),1)
KBUILD_CFLAGS += -DCAPSENSOR_READ_DATA_POLLING
endif
