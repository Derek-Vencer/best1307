...
typedef enum
{
    ...
    OTA_SET_UPGRADE_TYPE        = 0x9D,
    OTA_RSP_UPGRADE_TYPE        = 0x9E,
#ifdef UUID_TOTA_USE_OTA_EN
    OTA_TOTA_DATA               = 0x9F, //!< TOTA USE OTA   
#endif
    OTA_GET_SEGMENT_CHECK_LEN   = 0x7D,
...
#endif
#ifdef OTA_USE_DIFF
    bool  isDiffVersion;    //for TWS
    bool  isMasterImage;    //for TWS
#endif
/*****************************************************************************/
//                      OTA protocol version: 0.0.0.1                        //
/*****************************************************************************/
...
typedef struct {
    uint32_t magicNumber;               // BOOT_MAGIC_NUMBER or COPY_NEW_IMAGE
    uint32_t imageSize;
    uint32_t imageCrc;
...
void ibrt_ota_send_version_rsp(uint8_t* buf);
void ibrt_ota_set_version_diff_flag(bool isDiff);
